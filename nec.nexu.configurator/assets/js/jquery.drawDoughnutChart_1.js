...
;(function($, undefined) {
  $.fn.drawDoughnutChart = function(data, options) {
    var $this = this,
	  dummyValue = 1, /* this forces each pie piece to be rendered with the same size. to revert change, replace "dummyValue" with "data[i].value" */
      W = $this.width(),
      H = $this.height(),
      centerX = W/2,
      centerY = H/2,
    ...
    var $svg = $('<svg width="' + W + '" height="' + H + '" viewBox="0 0 ' + W + ' ' + H + '" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"></svg>').appendTo($this),
        $paths = [],
        $defs = $(document.createElementNS('http://www.w3.org/2000/svg', 'defs')).prependTo($svg),
    ...
    for (var i = 0, len = data.length; i < len; i++) {
      segmentTotal += dummyValue;
	  
	  
	  var fill = (data[i].Pattern && data[i].Pattern.Id) ? 'url(#pattern1234)' : '#' + data[i].Hex.replace('#', '');


      if (data[i].Pattern && data[i].Pattern.Id) {
        var pattern = document.createElementNS('http://www.w3.org/2000/svg', 'pattern');
            pattern.setAttribute('id','pattern1234');
            pattern.setAttribute('patternUnits',"userSpaceOnUse");
            pattern.setAttribute('width',"100");
            pattern.setAttribute('height',"100");
            pattern.setAttribute('viewbox',"0 0 100 100");
            pattern.setAttribute('overflow','visible');
        $pattern = $(pattern).appendTo($defs);
        var image = document.createElementNS('http://www.w3.org/2000/svg', 'image');
            image.setAttribute('overflow',"visible");
            image.setAttribute('width',"100");
            image.setAttribute('height',"100");
            image.setAttributeNS('http://www.w3.org/1999/xlink','xlink:href',data[i].Pattern.Thumbnail.FilePath);
            pattern.appendChild(image);
            $image = $(image).appendTo($pattern);
      }
    ...
    function pathMouseEnter(e) {
      ...
      $('.donutColorTip').show().text(data[order].DisplayName);
	...
	function pathClick(e){
		...
		settings.onPathClick.apply($(this),[e,data]); // old code, new version commented out below allows for individual path actions i think -amo
		/*var order = $(this).data().order;
    ...
    function drawPieSegments (animationDecimal) {
      ...
      //If data have only one value, we draw hollow circle(#1). !!!!!!! uncomment the commented calculation below, and remove 1 if reverting code
      if (data.length === 1 && (4.7122 < (rotateAnimation * ( /*(data[0].value / segmentTotal)*/ 1 * (PI * 2)) + startRadius))) {
        $paths[0].attr("d", getHollowCirclePath(doughnutRadius, cutoutRadius));
        return;
      }
      for (var i = 0, len = data.length; i < len; i++) {
        var segmentAngle = rotateAnimation * ((dummyValue / segmentTotal) * (PI * 2)),
